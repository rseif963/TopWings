  ...
  return (
    <div className='px-4 pt-10 md:px-10 w-full'>
      ...
      <div className='max-w-3xl w-full rounded-md overflow-hidden border border-borderColor mt-6'>
        <table className='w-full border-collapse text-left text-sm text-grey-600'>
            ...
            {bookings.map((booking, index)=>(
              <tr key={index} className='border-t border-borderColor text-grey-500'>
                <td className='p-3 flex items-center gap-3'>
                  ...
                  <p className='font-medum max-md:hidden'>{booking.car.brand} {booking.car.model}</p>
                </td>
                ...
                <td className='p-3'>
                  {booking.status === 'pending' ? (
                    <select onChange={e=> changeBookingStatus(booking._id, e.target.value)} value={booking.status} className='px-2 py-1.5 mt-1 text-grey-500 border border-borderColor rounded-md outline-none'>
